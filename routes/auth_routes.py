...
def auth_callback():
    ...
    try:
        ...
        if error:
            ...
            return redirect(url_for('index', error=f"Login failed: {error_description}"))
        ...
        if not token_response:
            return redirect(url_for('index', error="Token acquisition failed"))
        
        # Get ID token
        id_token = token_response.get('id_token')
        if not id_token:
            return redirect(url_for('index', error="No ID token received"))
        
        # Verify token and get email
        valid, email, error_msg = AuthService.verify_monmouth_token(id_token)
        if not valid:
            print(f"Token verification failed: {error_msg}")
            return redirect(url_for('index', error=f"Verification failed: {error_msg}"))
